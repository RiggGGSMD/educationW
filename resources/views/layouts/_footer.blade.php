<footer class="footer">
    <div class="container">
        <p class="pull-left">
            由 Rigg设计和编码 <span style="color: #e27575;font-size: 14px;">❤</span>
        </p>

        <p class="pull-right"><a href="https://weibo.com/u/6336066897/home?wvr=5&lf=reg"target="_blank"><img src="{{asset('/photos/Weibo_hover.png')}}"/></a></p>
        <p class="pull-right"><a href="mailto:name@email.com"><img src="{{asset('/photos/mail2_hover.png')}}"/></a></p>
        <p class="pull-right"><a href="https://github.com/RiggGGSMD/educationsys"target="_blank"><img src="{{asset('/photos/g-google_hover.png')}}"/></a></p>
    </div>
</footer>